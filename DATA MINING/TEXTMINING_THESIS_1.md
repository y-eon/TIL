...
class NaverSpider(scrapy.Spider):
    ...
    def start_requests(self):
        start_urls = 'https://search.naver.com/search.naver?&where=news&query=%EA%B8%88%EB%A6%AC&sm=tab_pge&sort=0&photo=0&field=0&reporter_article=&pd=3&ds=2005.01.01&de=2005.01.01&docid=&nso=so:r,p:from20050101to20050101,a:all&mynews=1&cluster_rank=15&start=1&refresh_start=0'
        yield scrapy.Request(url = start_urls, cookies={'news_office_checked':'1001,1018,2227'}, callback = self.chg_date)
    ...
    def news_name(self, response):
        li_list = response.xpath('//*[@id="main_pack"]/div[2]/ul/li')
        for i in li_list:
            news_name = i.xpath('./dl/dd[1]/span[1]/text()').extract()
            # if news_name in ['이데일리','연합뉴스']:
            #     href = li_list.xpath('./dl/dd/a/@href').get()
            #     url = response.urljoin(href[0].get())
            #     print('-------------------------------------------------------------------------------------')
            #     print(url)
            #     print('-------------------------------------------------------------------------------------')
            #     yield scrapy.Request(url = url, callback=self.parse_page_edaily)

            # elif news_name == '이데일리':
            #     href = li_list.xpath('./dd/a/@href')
            #     url = response.urljoin(href[0].extract())
            #     yield scrapy.Request(url = url, callback=self.parse_page_edaily)
            
            # elif news_name == '연합인포맥스':
            #     href = li_list.xpath('./dt/a/@href')
            #     url = response.urljoin(href[0].extract())
            #     yield scrapy.Request(url = url, callback=self.parse_page_yeonhapinfo)
            print('-------------------------------------------------------------------------------------')
            print(news_name)
            print('-------------------------------------------------------------------------------------')

    def parse_page_edaily(self, response):
        item = NaverCrawlerItem()
        item['source'] = '이데일리'
        item['title'] = response.xpath('//*[@id="articleTitle"]/text()').extract()
        item['date'] = response.xpath('//*[@id="main_content"]/div[1]/div[3]/div/span[3]/text()').extract()
        item['article'] = response.xpath("./span[@class='_sp_each_source']/text()").get().strip()
        yield item
    ...
    #     item['date'] = response.xpath('//*[@id="main_content"]/div[1]/div[3]/div/span[3]/text()').get()
    #     item['article'] = response.xpath('//*[@id="articleBodyContents"]').get()
    #     yield item
```
